config = \
{
    ...
    "webRoute":{
        ...
        "urlLength":6,
        # 404时将会呈现的图片的链接列表
        # 当短链接失效后，其他人访问该链接会从模板中渲染返回404.html，默认只有文字
        # 你可以选择在下面的列表中添加一个或者多个图片链接（表情包
        # 渲染404页面时将会随机抽取一张图片链接渲染到网页中并返回
        "404img":[]
    },
    # 自动回复功能
    # 当在匹配的情景下接收到匹配正则表达式的语句时 则会触发自动回复
    "corpus":{
        # 功能总开关
        "enable": True,
        # 规则列表
